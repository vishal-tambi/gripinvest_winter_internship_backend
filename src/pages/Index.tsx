...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-background via-muted/30 to-background">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 relative z-10">
          <div className="text-center space-y-8">
            ...
            <div className={`flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center px-4 sm:px-0 transition-all duration-1000 delay-700 ${
              animateHero ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
            }`}>
              <Link to="/auth?mode=signup" className="w-full sm:w-auto">
                <Button size="lg" variant="hero" className="w-full shadow-glow hover:shadow-glow-intense transition-all">
                  Start Investing
                  <ChevronRight className="ml-2 h-4 w-4" />
                </Button>
              </Link>
              <Link to="/auth" className="w-full sm:w-auto">
                <Button size="lg" variant="outline" className="w-full hover:shadow-elegant transition-all">
                  Sign In
                </Button>
              </Link>
            </div>
      ...
      {/* Stats Section with COOL ANIMATIONS! */}
      <section id="stats-section" className="py-16 bg-muted/30 relative overflow-hidden">
        {/* Animated background elements */}
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-10 left-10 w-32 h-32 bg-primary/20 rounded-full blur-3xl animate-pulse"></div>
          <div className="absolute bottom-10 right-10 w-40 h-40 bg-success/20 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }}></div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {stats.map((stat, index) => {
              ...
              const statKeys = ['investors', 'invested', 'returns', 'products'] as const;
              const statKey = statKeys[index];
              const animatedValue = countedStats[statKey];

              return (
                <Card
                  key={index}
                  className={`text-center shadow-card stat-card group cursor-pointer ${
                    animateStats
                ...
                >
                  <CardContent className="p-6 relative">
                    ...
                    <div className="absolute inset-0 gradient-animated opacity-0 group-hover:opacity-5 rounded-lg transition-opacity duration-500"></div>
                    ...
                    <div className="relative z-10">
                      <div className="inline-flex items-center justify-center w-16 h-16 bg-primary/10 rounded-full mb-4 group-hover:scale-110 transition-transform duration-300">
                        <Icon className={`h-8 w-8 text-primary group-hover:animate-icon-bounce ${
                          animateStats ? 'animate-icon-bounce' : ''
                        }`} style={{ animationDelay: `${index * 200 + 500}ms` }} />
                      </div>

                      {/* Animated counter */}
                      <div className={`text-3xl font-bold mb-2 group-hover:text-primary transition-colors ${
                        animateStats ? 'animate-count-up' : ''
                      }`} style={{ animationDelay: `${index * 100 + 800}ms` }}>
                        {animateStats ? formatStatValue(statKey, animatedValue) : '0'}
                      </div>

                      {/* Label with fade in */}
                      <div className={`text-sm text-muted-foreground group-hover:text-foreground transition-colors ${
                        animateStats ? 'animate-count-up' : 'opacity-0'
                    ...
                    <div className="absolute inset-0 pointer-events-none">
                      <div className="absolute top-2 right-2 w-2 h-2 bg-primary/30 rounded-full opacity-0 group-hover:opacity-100 group-hover:animate-ping transition-opacity"></div>
                      <div className="absolute bottom-2 left-2 w-1 h-1 bg-success/30 rounded-full opacity-0 group-hover:opacity-100 group-hover:animate-ping transition-opacity" style={{ animationDelay: '0.2s' }}></div>
                      <div className="absolute top-1/2 left-2 w-1.5 h-1.5 bg-warning/30 rounded-full opacity-0 group-hover:opacity-100 group-hover:animate-ping transition-opacity" style={{ animationDelay: '0.4s' }}></div>
                    </div>
                  </CardContent>
                </Card>
              );
          ...
          {/* Call-to-action appearing after stats animation */}
          {animateStats && (
            <div className="text-center mt-12 animate-slide-in-up" style={{ animationDelay: '1.5s', animationFillMode: 'forwards', opacity: 0 }}>
              <p className="text-lg text-muted-foreground mb-4">
                Join thousands of successful investors
              </p>
              <Link to="/auth?mode=signup">
                <Button variant="hero" size="lg" className="shadow-glow hover:shadow-glow-intense transition-all">
                  Start Your Journey
                  <ChevronRight className="ml-2 h-4 w-4" />
                </Button>
              </Link>
